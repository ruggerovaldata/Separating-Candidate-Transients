...
def GetImg1Srcs(session,dataset_id):
    ...
    rms_min = [images[i].rms_min for i in range(len(images))]
    Imgs_idx = []
    for i, val in enumerate(Imgs):
        Imgs_idx.append([val, rms_min[i]])
    Imgs_idx.sort()
    found = False
    for i, val in enumerate(Imgs_idx):
        if val[1] != None: 
            found = True
            idx = val[0]
        if found:
            break
    ExtractedSrcs = session.query(Extractedsource, Assocxtrsource).select_from(join(Extractedsource, Assocxtrsource)).filter(Extractedsource.image_id == idx).all()
